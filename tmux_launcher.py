...
class TmuxLauncher():
    ...
    def commands(self):
        ...
        return ["python -u tools/train.py " + str(opt) for opt in opts]

    def test_commands(self):
        opts = self.test_options()
        return ["python -u tools/test.py " + str(opt) for opt in opts]
    ...
    def prepare_tmux_panes(self, num_experiments, dry=False):
        ...
        for w in range(self.n_windows):
            if dry:
                continue
            window_name = f"{self.exp_name}_{w}"
            os.system("tmux new -d -s {} -n w".format(window_name))
            # os.system("tmux new-window -s {}".format(window_name))
    ...
    def send_command(self, exp_id, command, dry=False, continue_train=False):
        ...
        pane_name = f"{self.exp_name}_{exp_id // self.pane_per_window}"
    ...
    def run_command(self, command, ids, resume_iter=None, continue_train=False, gpu_id=None):
        ...
        for id in ids:
            ...
            refined_command = self.refine_command(this_command, resume_iter, continue_train=continue_train,
                                                  gpu_id=gpu_id)
            num_repeats = 1
    ...
    def launch(self, ids, test=False, dry=False, continue_train=False):
        ...
        commands = ["conda activate seg_env"] + commands

        if type(ids) is list:
            commands = [commands[i] for i in ids]
        if not self.tmux_prepared:
            # self.prepare_tmux_panes(len(commands), dry)
            self.prepare_tmux_panes(1, dry)
            assert self.tmux_prepared

        for i, command in enumerate(commands):
            self.send_command(0, command, dry, continue_train=continue_train)

    def dry(self):
        self.launch(dry=True, ids=0)
    ...
    def create_comparison_html(self, expr_name, ids, subdir, title, phase):
        ...
        command = "python ~/tools/html.py --web_dir_prefix results/comparison_ --name %s --dirs %s --labels %s --image_width 256" % (
            expr_name + '_' + title, dirstr, labelstr)
        print(command)
        os.system(command)
